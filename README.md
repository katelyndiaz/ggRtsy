...
color palette to the user’s repertoire. This is the goghColors dataset,
which contains the RGB and hex codes of colors picked from Van Gogh
paintings. It also has a function that work alongside `ggplot2()` to
create more interesting data visualizations and add contextual
...
![](README_files/figure-gfm/unnamed-chunk-3-1.png)<!-- --> Continuous
example using the `sunflowers` color palette:
...
![](README_files/figure-gfm/unnamed-chunk-4-1.png)<!-- --> Discrete
example using the `almondBlossoms` color palette:

``` r
ggplot(storms, aes(x = hour, y = wind, color = status)) +
  geom_point(size = 4) +
  scale_color_gogh(palette = "almondBlossoms", discrete = TRUE, reverse=FALSE) +
  theme_minimal()
```
